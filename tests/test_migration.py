...
def test_migrate_multi_session_backfill_targets_last(tmp_path):
    import migrate_to_sqlite
    import db as db_mod

    json_path = str(tmp_path / "salary_data.json")
    db_path = str(tmp_path / "salary.db")
    # Two truncated sessions: 1h29m + 0h30m = 1h59m base, but the precise
    # day total is 2.0h. The 1m remainder must land on the LAST session only.
    data = {
        "42": {
            "rate": 500.0,
            "work_sessions": {
                "2026-07-04": {
                    "total_hours": 2.0, "total_earnings": 1000.0,
                    "sessions": [
                        {"hours": 1, "minutes": 29, "earnings": 750.0,
                         "timestamp": "2026-07-04T10:00:00", "source": "clickup",
                         "clickup_id": "e1", "task_name": "T1",
                         "project_name": "P", "description": ""},
                        {"hours": 0, "minutes": 30, "earnings": 250.0,
                         "timestamp": "2026-07-04T12:00:00", "source": "clickup",
                         "clickup_id": "e2", "task_name": "T2",
                         "project_name": "P", "description": ""},
                    ],
                }
            },
            "clickup_synced_entries": ["e1", "e2"],
            "clickup_settings": {},
        }
    }
    ...
    # No WAL sidecar litter left behind (guards the _remove_with_sidecars fix).
    leftovers = [p.name for p in tmp_path.iterdir()
                 if p.name.endswith(".tmp-wal") or p.name.endswith(".tmp-shm")]
    assert leftovers == []

    conn = db_mod.get_connection(db_path)
    rows = conn.execute(
        "SELECT clickup_id, duration_ms FROM work_sessions "
        "WHERE user_id = ? ORDER BY id", ("42",),
    ).fetchall()
    conn.close()
    by_id = {r["clickup_id"]: r["duration_ms"] for r in rows}

    base_e1 = (1 * 60 + 29) * 60 * 1000   # 5_340_000
    base_e2 = (0 * 60 + 30) * 60 * 1000   # 1_800_000
    # First (non-last) session keeps its exact base duration.
    assert by_id["e1"] == base_e1
    # Last session absorbed the delta -> larger than its base.
    assert by_id["e2"] > base_e2
    # Day total restored to the precise 2.0h.
    total_ms = by_id["e1"] + by_id["e2"]
    assert abs(total_ms / 3_600_000 - 2.0) < 1e-3
