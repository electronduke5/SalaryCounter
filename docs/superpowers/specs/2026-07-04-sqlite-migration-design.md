...
**Потоки внутри процесса.** FastAPI/`TestClient` могут исполнять обработчики в разных потоках, а один `sqlite3.Connection` по умолчанию нельзя использовать вне создавшего потока (`ProgrammingError: SQLite objects created in a thread…`). Поэтому соединение открывается с `check_same_thread=False`, а `DataManager` сериализует доступ к нему через `threading.Lock` (каждый метод, трогающий `self.conn`, — под локом; для `add_synced_session` лок держится на всю транзакцию). WAL остаётся для межпроцессной конкуренции; лок закрывает внутрипроцессную многопоточность.
